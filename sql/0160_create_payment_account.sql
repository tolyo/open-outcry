...
CREATE OR REPLACE FUNCTION create_payment_account(app_entity_id_param text, currency_param text)
...
BEGIN
    ...
    FROM
        app_entity
    WHERE
        pub_id = app_entity_id_param INTO app_entity_instance;
    IF NOT FOUND THEN
    ...
    FROM
        currency
    WHERE
        name = currency_param INTO currency_instance;
    IF NOT FOUND THEN
    ...
    WHERE
        ...
        AND app_entity_id = app_entity_instance.id INTO payment_account_instance;
    IF NOT FOUND THEN
        -- create payment_account
        INSERT INTO payment_account(app_entity_id, currency_name)
