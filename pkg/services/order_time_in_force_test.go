...
func (assert *ServiceTestSuite) TestGtc() {
	...
	tradeOrder, _ := ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", models.Sell, 1.0, 10.0, "GTC")
	...
	assert.Equal(10.0, models.FindCurrencyAccountByAppEntityIdAndCurrencyName(entity, "BTC").AmountReserved)
}
...
func (assert *ServiceTestSuite) TestFok() {
	...
	tradeOrder, _ := ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", models.Sell, 1.0, 1.0, models.FOK)
	...
	assert.Equal(0.0, models.FindCurrencyAccountByAppEntityIdAndCurrencyName(entity, "BTC").AmountReserved)

	//when: given a new order that cannot be filled even when other orders present
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", "LIMIT", models.Buy, 1.0, 1.0, "GTC")

	tradeOrder, _ = ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", models.Sell, 1.0, 2.0, models.FOK)
	...
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", models.Market, models.Buy, 0.0, 2.0, "GTC")
...
func (assert *ServiceTestSuite) TestIoc() {
	...
	// when: given a new order that cannot be filled
	tradeOrder, _ := ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", models.Sell, 1.0, 1.0, models.IOC)
	...
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", "LIMIT", models.Buy, 1.0, 1, "GTC")
	tradeOrder, _ = ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", models.Sell, 1.0, 2, models.IOC)
