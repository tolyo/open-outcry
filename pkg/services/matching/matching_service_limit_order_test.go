...
func (assert *ServiceTestSuite) TestProcessLimitSellOrderSave() {
	...
	res, _ := ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Sell, 10.0, 100.0, "GTC")

	// then: a matching unit should save the trade order on save order to the order book
	assert.NotNil(res)
	assert.Equal(1, GetSellBookOrderCount())
	assert.Equal([]PriceVolume{{Price: 10.0, Volume: 100.0}}, GetVolumes("BTC_EUR", Sell))
}

func (assert *ServiceTestSuite) TestProcessLimitBuyOrderSave() {
	// when: a limit order is sent to an empty matching unit
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Buy, 10, 100, "GTC")

	// then: a matching unit should save the orderSELL
	assert.Equal(1, GetBuyBookOrderCount())
	assert.Equal([]PriceVolume{{Price: 10.0, Volume: 100.0}}, GetVolumes("BTC_EUR", Buy))
}
...
func (assert *ServiceTestSuite) TestProcessLimitNoMatchCaseIncomingBuy() {
	...
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Sell, 10, 100, "GTC")
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", "LIMIT", Buy, 9, 100, "GTC")
	...
	assert.Equal([]PriceVolume{{Price: 10.0, Volume: 100.0}}, GetVolumes("BTC_EUR", Sell))
	assert.Equal([]PriceVolume{{Price: 9.0, Volume: 100.0}}, GetVolumes("BTC_EUR", Buy))
}
...
func (assert *ServiceTestSuite) TestProcessLimitNoMatchCaseIncomingSell() {
	...
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Buy, 9, 100, "GTC")
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", "LIMIT", Sell, 10, 100, "GTC")
...
func (assert *ServiceTestSuite) TestProcessLimitExactMatchIncomingBuy() {
	...
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", "LIMIT", Buy, 10, 100, "GTC")
...
func (assert *ServiceTestSuite) TestProcessLimitPartialMatchIncomingBuySingleTrade() {
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Sell, 10.00, 100.00, "GTC")
	...
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", "LIMIT", Buy, 10.00, 50.00, "GTC")
	...
	assert.Equal([]PriceVolume{{Price: 10.0, Volume: 50.0}}, GetVolumes("BTC_EUR", Sell))
	assert.Equal([]PriceVolume{}, GetVolumes("BTC_EUR", Buy))
}

func (assert *ServiceTestSuite) TestProcessLimitOverflowMatchIncomingBuySingleTrade() {
	// when: there is a SELL order in the book
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Sell, 10.00, 10.00, "GTC")
	...
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", "LIMIT", Buy, 10.00, 15, "GTC")
	...
	assert.Equal([]PriceVolume{{Price: 10.00, Volume: 5.0}}, GetVolumes("BTC_EUR", Buy))
}

func (assert *ServiceTestSuite) TestProcessLimitPartialMatchIncomingSellSingleTrade() {
	// when: there is a BUY order
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Buy, 10.00, 100.00, "GTC")
	...
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", "LIMIT", Sell, 10.00, 50.00, "GTC")
	...
	assert.Equal([]PriceVolume{{Price: 10.00, Volume: 50.0}}, GetVolumes("BTC_EUR", Buy))
}
...
func (assert *ServiceTestSuite) TestProcessLimitOverflowMatchIncomingSellSingleTrade() {
	...
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", "LIMIT", Sell, 10.00, 150.00, "GTC")
	...
	assert.Equal([]PriceVolume{{Price: 10.00, Volume: 50.0}}, GetVolumes("BTC_EUR", Sell))
...
func (assert *ServiceTestSuite) TestProcessLimitExactMatchIncomingBuysMultipleTrades() {
	...
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", "LIMIT", Buy, 10.00, 5.00, "GTC")
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", "LIMIT", Buy, 10.00, 5.00, "GTC")
...
func (assert *ServiceTestSuite) TestProcessLimitExactMatchIncomingSellMultipleTrades() {
	...
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Buy, 10.00, 10.00, "GTC")
	...
	// when: incoming buy order is only partially matched
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", "LIMIT", Sell, 10.00, 5.00, "GTC")
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", "LIMIT", Sell, 10.00, 5.00, "GTC")
...
func (assert *ServiceTestSuite) TestProcessLimitPartialMatchMultipleBookSellsToMultipleTrades() {
	...
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Sell, 10.00, 50.00, "GTC")
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Sell, 10.00, 50.00, "GTC")
	...
	// when: incoming buy order is only partially matched
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", "LIMIT", Buy, 10.00, 75.00, "GTC")
	...
	assert.Equal([]PriceVolume{{Price: 10.00, Volume: 25.0}}, GetVolumes("BTC_EUR", Sell))
	assert.Equal([]PriceVolume{}, GetVolumes("BTC_EUR", Buy))
}
...
func (assert *ServiceTestSuite) TestProcessLimitPartialMatchMultipleBookBuysToMultipleTrades() {
	...
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Buy, 10.00, 50.00, "GTC")
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Buy, 10.00, 50.00, "GTC")
	...
	// when: incoming SELL order is only partially matched
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", "LIMIT", Sell, 10.00, 75.00, "GTC")
	...
	assert.Equal([]PriceVolume{{Price: 10.00, Volume: 25.0}}, GetVolumes("BTC_EUR", Buy))
}
...
func (assert *ServiceTestSuite) TestProcessLimitExactMatchMultipleBookSellsToMultipleTrades() {
	...
	// when: incoming buy order is only partially matched
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", "LIMIT", Buy, 10.00, 40.00, "GTC")
...
func (assert *ServiceTestSuite) TestProcessLimitExactMatchMultipleBookBuyToMultipleTrades() {
	...
	// when:
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Buy, 10.00, 5.00, "GTC")
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Buy, 10.00, 5.00, "GTC")
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Buy, 10.00, 5.00, "GTC")
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Buy, 10.00, 5.00, "GTC")
	...
	// when: incoming buy order is only partially matched
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", "LIMIT", Sell, 10.00, 20.00, "GTC")
...
func (assert *ServiceTestSuite) TestProcessLimitIncompleteMatchMultipleBookSellsToMultipleTrades() {
	...
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Sell, 10.00, 5.00, "GTC")
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Sell, 10.00, 5.00, "GTC")
	...
	// when: incoming buy order is only partially matched
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", "LIMIT", Buy, 10.00, 17, "GTC")
	...
	assert.Equal([]PriceVolume{{Price: 10.00, Volume: 7.0}}, GetVolumes("BTC_EUR", Buy))
}
...
func (assert *ServiceTestSuite) TestProcessLimitIncompleteMatchMultipleBookBuysToMultipleTrades() {
	...
	// when: incoming SELL order is only partially matched
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", "LIMIT", Sell, 10.00, 175.00, "GTC")
	...
	assert.Equal([]PriceVolume{{Price: 10.00, Volume: 75.0}}, GetVolumes("BTC_EUR", Sell))
...
func (assert *ServiceTestSuite) TestProcessLimitPartialMatchMultipleBookSellsToMultipleTradesMultiplePrices() {
	...
	// when: there are multiple SELL orders in the book and BUY limit order arrive that cross but can fill only partially one of the orders
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Sell, 9.00, 50.00, "GTC")
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Sell, 10.00, 50.00, "GTC")
	...
	// when: incoming buy order is only partially matched
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", "LIMIT", Buy, 11.00, 75.00, "GTC")
...
func (assert *ServiceTestSuite) TestProcessLimitPartialMatchMultipleBookBuysToMultipleTradesMultiplePrices() {
	...
	// when: there are multiple BUY orders in the book and SELL limit order arrive that cross but can fill only partially one of the orders
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Buy, 9.00, 50.00, "GTC")
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Buy, 10.00, 50.00, "GTC")
	...
	// when: incoming SELL order is only partially matched
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", "LIMIT", Sell, 8.00, 75.00, "GTC")
	...
	assert.Equal([]PriceVolume{{Price: 9.00, Volume: 25.0}}, GetVolumes("BTC_EUR", Buy))
}
