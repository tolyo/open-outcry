...
//
// then:
//    assert.Equal(GetVolumeAtPrice("BTC_EUR", "SELL", 10.6) |> Decimal.to_float() ==
//             100
//
//    assert.Equal(db.QueryVal(`
//           SELECT SUM(volume)
//             FROM price_level
//             WHERE side = 'SELL'
...
//
// then:
//    assert.Equal(GetVolumeAtPrice("BTC_EUR", "BUY", 9.5) |> Decimal.to_float() ==
//             100
//
//    assert.Equal(db.QueryVal(`
//             SELECT SUM(volume)
