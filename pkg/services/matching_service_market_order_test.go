...
func (assert *ServiceTestSuite) TestProcessMarketSellOrderSave() {
	...
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", Market, Sell, 0, 100, "GTC")
...
func (assert *ServiceTestSuite) TestProcessMarketBuyOrderSave() {
	// when: a market order is sent to an empty matching unit
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", Market, Buy, 0, 100, "GTC")
...
func (assert *ServiceTestSuite) TestProcessMarketSellOrderWithExistingSellLimit() {
	...
	assert.Equal([]PriceVolume{{Price: 10, Volume: 100}}, GetVolumes("BTC_EUR", Sell))
}

func (assert *ServiceTestSuite) TestProcessMarketBuyOrderWithExistingBuyLimit() {

	// when: a market order is sent to an non empty matching unit
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Buy, 10, 10, "GTC")
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", Market, Buy, 0, 10, "GTC")

	// then: a matching unit should save the trade order but it should not be visible to the order book
	assert.Equal(2, GetBuyBookOrderCount())
	assert.Equal([]PriceVolume{{Price: 10.0, Volume: 10.0}}, GetVolumes("BTC_EUR", Buy))
}
...
func (assert *ServiceTestSuite) TestProcessMarketSellOrderWithExistingBuyLimit() {
	...
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", Market, Sell, 0, 100, "GTC")
...
func (assert *ServiceTestSuite) TestProcessMarketBuyOrderWithExistingSellLimit() {
	// when:
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Sell, 10, 100, "GTC")
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", Market, Buy, 0, 100, "GTC")
	// then:
	assert.Equal(1, GetTradeCount())
	assert.Equal([]float64{10.0}, GetTradePrices())
	assert.Equal(0, GetBuyBookOrderCount())
	assert.Equal(1, GetSellBookOrderCount())
	assert.Equal([]PriceVolume{}, GetVolumes("BTC_EUR", Buy))
	assert.Equal([]PriceVolume{{Price: 10.0, Volume: 90.0}}, GetVolumes("BTC_EUR", Sell))
}

func (assert *ServiceTestSuite) TestProcessMarketSellOrderWithMultipleExistingBuyLimits() {
	// when:
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Buy, 5, 10, "GTC")
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Buy, 4, 10, "GTC")
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Buy, 7, 10, "GTC")
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", Market, Sell, 0, 30, "GTC")
...
func (assert *ServiceTestSuite) TestProcessMarketBuyOrderWithMultipleExistingSellLimits() {
	...
	// when:
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Sell, 5, 10, "GTC")
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Sell, 4, 10, "GTC")
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Sell, 7, 10, "GTC")
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", Market, Buy, 0, 160, "GTC")
...
func (assert *ServiceTestSuite) TestProcessLimitBuyOrderWithExistingMarketSell() {
	...
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", Market, Sell, 0, 10, "GTC")
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Buy, 5, 10, "GTC")
...
func (assert *ServiceTestSuite) TestProcessLimitSellOrderWithExistingMarketBuy() {
	...
	// when: the opposite book {es not have outstanding limit orders
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", Market, Buy, 0, 10, "GTC")
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Sell, 5, 10, "GTC")
	...
	assert.Equal(1, GetSellBookOrderCount())
	assert.Equal([]PriceVolume{}, GetVolumes("BTC_EUR", Buy))
	assert.Equal([]PriceVolume{{Price: 5.0, Volume: 8.0}}, GetVolumes("BTC_EUR", Sell))
}
...
func (assert *ServiceTestSuite) TestProcessLimitBuyOrderWithExistingMarketSellAndBetterLimitSell() {
	...
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", "LIMIT", Sell, 5, 10, "GTC")
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", Market, Sell, 0, 10, "GTC")
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Buy, 6, 20, "GTC")
...
func (assert *ServiceTestSuite) TestProcessLimitSellOrderWithExistingMarketBuyAndBetterLimitBuy() {
	...
	// when: the opposite book has a market and a limit order
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", "LIMIT", Buy, 5, 10, "GTC")
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", Market, Buy, 0, 50, "GTC")
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Sell, 4, 20, "GTC")
...
func (assert *ServiceTestSuite) TestProcessLimitBuyOrderWithExistingMarketSellAndWorseLimitSell() {
	...
	// when: the opposite book has a market and a limit order
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", "LIMIT", Sell, 7, 10, "GTC")
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", Market, Sell, 0, 10, "GTC")
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Buy, 6, 10, "GTC")
	...
	assert.Equal(1, GetSellBookOrderCount())
	assert.Equal([]PriceVolume{}, GetVolumes("BTC_EUR", Buy))
	assert.Equal([]PriceVolume{{Price: 7.0, Volume: 10.0}}, GetVolumes("BTC_EUR", Sell))
}

func (assert *ServiceTestSuite) TestProcessLimitSellOrderWithExistingMarketBuyAndWorseLimitBuy() {

	// when: the opposite book has a market and a limit order
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", "LIMIT", Buy, 4, 10, "GTC")
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", Market, Buy, 0, 100, "GTC")
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Sell, 5, 20, "GTC")
	...
	assert.Equal(1, GetBuyBookOrderCount())
	assert.Equal(0, GetSellBookOrderCount())
	assert.Equal([]PriceVolume{{Price: 4.0, Volume: 10.0}}, GetVolumes("BTC_EUR", Buy))
	assert.Equal([]PriceVolume{}, GetVolumes("BTC_EUR", Sell))
}
...
func (assert *ServiceTestSuite) TestProcessMarketBuyOrderWithExistingMarketSellAndNoReferencePrice() {
	...
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", Market, Sell, 0, 10, "GTC")
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", Market, Buy, 0, 50, "GTC")
...
func (assert *ServiceTestSuite) TestProcessMarketSellOrderWithExistingMarketBuyAndNoReferencePrice() {
	...
	// when: there is not a trade with a reference price
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", Market, Buy, 0, 50, "GTC")
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", Market, Sell, 0, 10, "GTC")
...
func (assert *ServiceTestSuite) TestProcessMarketBuOrderWithExistingMarketSellAndLimitSell() {
	...
	assert.Equal(1, GetSellBookOrderCount())
	assert.Equal([]PriceVolume{}, GetVolumes("BTC_EUR", Buy))
	assert.Equal([]PriceVolume{{Price: 5.0, Volume: 10.0}}, GetVolumes("BTC_EUR", Sell))
}
...
func (assert *ServiceTestSuite) TestProcessMarketSellOrderWithExistingMarketBuyAndLimitBuy() {
	...
	assert.Equal(1, GetBuyBookOrderCount())
	assert.Equal(0, GetSellBookOrderCount())
	assert.Equal([]PriceVolume{{Price: 5.0, Volume: 10.0}}, GetVolumes("BTC_EUR", Buy))
	assert.Equal([]PriceVolume{}, GetVolumes("BTC_EUR", Sell))
}
...
func (assert *ServiceTestSuite) TestProcessMarketBuyOrderWithExistingMarketSellAnReferencePrice() {
	...
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", "LIMIT", Sell, 5, 1, "GTC")
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Buy, 5, 1, "GTC")
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", Market, Sell, 0, 10, "GTC")
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", Market, Buy, 0, 50, "GTC")
...
func (assert *ServiceTestSuite) TestProcessMarketBuyOrderWithExistingMarketSellAndMultipleReferencePrices() {
	...
	// when: there is a trade with a reference price
	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", "LIMIT", Sell, 4, 1, "GTC")
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Buy, 4, 1, "GTC")

	db.Instance().Exec("UPDATE trade SET created_at = current_timestamp + interval '1 second' WHERE price = 4")

	ProcessTradeOrder(assert.instrumentAccount2, "BTC_EUR", "LIMIT", Sell, 6, 1, "GTC")
	ProcessTradeOrder(assert.instrumentAccount1, "BTC_EUR", "LIMIT", Buy, 6, 1, "GTC")
